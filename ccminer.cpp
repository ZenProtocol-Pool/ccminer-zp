...
enum sha_algos opt_algo = ALGO_KECCAK_ZP;
int opt_n_threads = 0;
int gpu_threads = 1;
int opt_affinity = -1;
int opt_priority = 0;
...
Options:\n\
  -a, --algo=ALGO zenprotocol sha-3 256\n\
...
{
	const char *body, *header, *target;

	int target_size;
	int atarget_sz = ARRAY_SIZE(work->target);
	int i;

	size_t data_size = jobj_binary(val, "header", work->data, sizeof(work->data));
	...
	//	work->target[i] = swab32((uint32_t)work->target + i);

	if (opt_debug) {
		char *hex_target;
		hex_target = abin2hex((unsigned char*)&work->target, 32);
		applog(LOG_WARNING, "Work target: %s", hex_target);
		free(hex_target);
	}

	json_t *jr = json_object_get(val, "noncerange");

	hex2bin((uchar*)work->noncerange.u64, "FFFFFFFF", 8);
	
	/* use work ntime as job id (solo-mining) */
	cbin2hex(work->job_id, (const char*)&work->data[18], 4);
...
static bool submit_upstream_work(CURL *curl, struct work *work) {
	...
	if (have_stratum)
	{
		uint32_t nonce_ex, nonce;
		char *nonceexstr, *noncestr;
		uint32_t sent = 0;

		le32enc(&nonce_ex, work->data[23]);
		le32enc(&nonce, work->data[24]);
		nonceexstr = bin2hex((const unsigned char *)(&nonce_ex), 4);
		...
		sprintf(s,
			"{\"method\": \"mining.submit\", \"params\": [\"%s\", \"%s\", \"%s\", \"0\", \"%s\"], \"id\":4}",
			rpc_user, work->job_id, nonceexstr, noncestr);
	...
	else
	{
		...
		applog(LOG_WARNING, "Submitting found block to daemon...");
		...
		sprintf(s,
			...
			str);
		if (opt_debug)
			applog(LOG_WARNING, "Submitting: %s", s);

		/* issue JSON-RPC request */
		val = json_rpc_ZP(curl, rpc_url, "/blockchain/submitheader", s,
			false, true, &curl_err);
...
{
	json_t *header = json_object_get(val, "header");
	if(!header)
	{
		allow_gbt = false;
		applog(LOG_INFO, "GBT not supported, block header unavailable");
		return false;
	}
	...
	if(!work->height)
	{
		...
		json_t *key = json_object_get(val, "blockNumber");
...
static bool get_blocktemplate(CURL *curl, struct work *work)
{
	...
	int curl_err = 0;
	json_t *val = json_rpc_ZP(curl, rpc_url, "/blockchain/blocktemplate", NULL, 
								 have_longpoll, false, &curl_err);
	...
	bool rc = gbt_work_decode(val, work);
...
static bool get_mininginfo(CURL *curl, struct work *work)
{
	...
	int curl_err = 0;
	json_t *val = json_rpc_ZP(curl, rpc_url, "/blockchain/info", NULL,
		have_longpoll, false, &curl_err);
	...
	if(!val && curl_err == -1)
	{
		...
		if(opt_debug)
		{
			applog(LOG_DEBUG, "/blockchain/info not supported");
		}
		return false;
	}
	else
	{
		...
		if(res)
		{
			json_t *key = json_object_get(val, "difficulty");
...
static bool get_upstream_work(CURL *curl, struct work *work)
{
	...
	val = json_rpc_ZP(curl, rpc_url, "/blockchain/blocktemplate", NULL,
		have_longpoll, false, &curl_err);
...
static bool stratum_gen_work(struct stratum_ctx *sctx, struct work *work) {
	...
	memcpy((uchar *)work->data, (uchar *)sctx->job.header, 100);

	work->nonce_extra = (uint32_t)((double)rand() / (1ULL + RAND_MAX) * 0xffffffffu);
	work->data[23] = work->nonce_extra;

	pthread_mutex_unlock(&sctx->work_lock);

	if (opt_debug) {
		applog(LOG_DEBUG, "DEBUG: job_id='%s' nonce_extra=%d", work->job_id, work->nonce_extra);
	}

	diff_to_target_zp(work->target, sctx->job.diff / (256.0 * opt_difficulty));
...
static void *miner_thread(void *userdata)
{
	...
	uint64_t loopcnt = 0;
	uint32_t max_nonce;
	uint64_t end_nonce = UINT32_MAX / opt_n_threads * (thr_id + 1) - (thr_id + 1); //0x100000000ull / opt_n_threads * (thr_id + 1) - 1;
	bool extrajob = false;
	char s[16];
	int rc = 0;
	...
	while(!stop_mining)
	{
		...
		nonceptr = (uint32_t*)(((char*)work.data) + 96);
		...
		if(have_stratum)
		{
			...
			if(nonceptr[0] >= end_nonce || extrajob)  //TEST THIS
			{
				extrajob = false;
				int loop = 0;
		...
		else
		{
			...
			if ((time(NULL) - g_work_time) >= scan_time || nonceptr[0] >= (end_nonce - 0x100))//Test this
			{
		...
		*    before hashrate is computed */
		...
		rc = scanhash_keccak256_zenprotocol(thr_id, work.data, work.target, max_nonce, &hashes_done);
		
		mining_has_stopped[thr_id] = true;
		/* record scanhash elapsed time */
		gettimeofday(&tv_end, NULL);
		if(rc && opt_debug)
			applog(LOG_NOTICE, CL_CYN "found1 => %08x" CL_GRN " %08x", nonceptr[0], swab32(nonceptr[0])); // data[19]

		if(rc > 1 && opt_debug)
				applog(LOG_NOTICE, CL_CYN "found2 => %08x" CL_GRN " %08x", nonceptr[2], swab32(nonceptr[2])); // data[21]
...
start:
	...
	while(!stop_mining)
	{
		...
		val = json_rpc_call(curl, lp_url, rpc_userpass, NULL,
							false, true, &err);
		if(have_stratum)
		{
			if(val)
