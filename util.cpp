...
static size_t resp_hdr_cb(void *ptr, size_t size, size_t nmemb, void *user_data)
{
	...
	{
		/* when available: X-Mining-Extensions: noncerange */
	}
out:
	free(key);
	free(val);
...
json_t *json_rpc_ZP(CURL *curl, const char *url,
	const char *path, const char *rpc_req,
	bool longpoll, bool expect_empty, int *curl_err)
{
	...
	snprintf(url_path, sizeof(url_path), "%s%s", url,path);
	/* it is assumed that 'curl' is freshly [re]initialized at this pt */

	curl_easy_setopt(curl, CURLOPT_ERRORBUFFER, curl_err_str);
	if (opt_protocol)
		curl_easy_setopt(curl, CURLOPT_VERBOSE, 1);
	rc = curl_easy_setopt(curl, CURLOPT_URL, url_path);
	if (rc != CURLE_OK)
	{
	...
	if (opt_protocol)
		applog(LOG_DEBUG, "JSON protocol request: %s%s \n%s", url, path, rpc_req);
	...
	if ((!all_data.buf || !all_data.len) && expect_empty) {
		databuf_free(&all_data);
		curl_slist_free_all(headers);
		curl_easy_reset(curl);
		return NULL;
	}
	else if (expect_empty && (all_data.buf || all_data.len) ) {
		
		char * errstr = (char*)malloc(all_data.len + 14);
		snprintf(url_path, sizeof(url_path), "{\"error\": \"%s\"}", (char*)all_data.buf);
		val = JSON_LOADS(errstr, &err);
		databuf_free(&all_data);
		curl_slist_free_all(headers);
		curl_easy_reset(curl);
		free(errstr);
	...
	if (*httpdata != '{' && *httpdata != '[')
	{
		...
		applog(LOG_ERR, "Error RPC Call: %ld.", errcode);
...
bool fulltest(const uint32_t *hash, const uint32_t *target)
{
	...
	for (i = 0; i < 8; i++) {
		if ( swab32(hash[i]) > swab32(target[i]) ) {
			rc = false;
			break;
		}
		if (swab32(hash[i]) < swab32(target[i]) ) {
		...
		if ((uint32_t*)hash[1] == (uint32_t*)target[1]) {
...
void diff_to_target_zp(uint32_t *target, double diff)
{
	uint64_t m;
	int k, i;
	uint32_t tmp_target[8];

	for (k = 6; k > 0 && diff > 1.0; k--)
		diff /= 4294967296.0;
	m = (uint64_t)(4294901760.0 / diff);
	if (m == 0 && k == 6)
		memset(tmp_target, 0xff, 32);
	else
	{
		memset(tmp_target, 0, 32);
		tmp_target[k] = (uint32_t)m;
		tmp_target[k + 1] = (uint32_t)(m >> 32);
	}
	swab256(&target[0], &tmp_target[0]);
	//for (i = 0; i < 8; i++) {
	//	be32enc(&target[i], tmp_target[7 - i]);
	//}
}
...
static bool stratum_notify_zp(struct stratum_ctx *sctx, json_t *params) {
	const char *job_id, *header, *nbits;
	bool clean, ret = false;;

	job_id = json_string_value(json_array_get(params, 0));
	header = json_string_value(json_array_get(params, 1));
	nbits = json_string_value(json_array_get(params, 2));
	clean = json_is_true(json_array_get(params, 3));

	if (!job_id || !header || !nbits || strlen(header) != 200) {
	...
	hex2bin((uchar *)sctx->job.header, header, 100);
	hex2bin(sctx->job.nbits, nbits, 4);
	sctx->job.clean = clean;

	sctx->job.diff = sctx->next_diff;

	sctx->job.height = sctx->job.header[9];
...
static bool stratum_notify(struct stratum_ctx *sctx, json_t *params)
{
	...
	if (opt_algo == ALGO_KECCAK_ZP) {
		return stratum_notify_zp(sctx, params);
	}
...
void print_hash_tests(void)
{
	...
	keccak256_zenprotocol_hash(&hash[0], &buf[0]);
	printpfx("ZenProtocol", hash);
...
void printhex(const unsigned char *p, size_t len) {
	int i;
	for (i = 0; i < len; i++)
